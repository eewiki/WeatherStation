...
run () {
	...
	do
		...
		if [ "x$solar_volt" != "x" ] && [ "x$solar_mamp" != "x" ] ; then
			...
			if [ 1 -eq "$(echo "${solar_mamp} > 0" | bc)" ] ; then
				...
				if [ -f /home/voodoo/send.data ] ; then
					curl -X POST "http://192.168.11.252:8100/v1/data/for/Solar?volt=${solar_volt}&mamp=${solar_mamp}"
				fi
			else
		...
		if [ "x$battery_volt" != "x" ] && [ "x$battery_mamp" != "x" ] ; then
			...
			if [ -f /home/voodoo/send.data ] ; then
				curl -X POST "http://192.168.11.252:8100/v1/data/for/Battery?volt=${battery_volt}&mamp=${battery_mamp}"
			fi
		fi
		...
		if [ "x$fivev_volt" != "x" ] && [ "x$fivev_mamp" != "x" ] ; then
			...
			if [ -f /home/voodoo/send.data ] ; then
				curl -X POST "http://192.168.11.252:8100/v1/data/for/5VRail?volt=${fivev_volt}&mamp=${fivev_mamp}"
			fi
		fi
		...
		if [ "x$twelvev_volt" != "x" ] && [ "x$twelvev_mamp" != "x" ] ; then
			...
			if [ -f /home/voodoo/send.data ] ; then
				curl -X POST "http://192.168.11.252:8100/v1/data/for/12VRail?volt=${twelvev_volt}&mamp=${twelvev_mamp}"
			fi
		fi
		...
		if [ "x$battery_hdc_temp" != "x" ] ; then
			if [ 0 -eq "$(echo "${battery_hdc_temp} > ${max_temperature}" | bc)" ] ; then
				...
				if [ -f /home/voodoo/send.data ] ; then
					curl -X POST "http://192.168.11.252:8100/v1/data/for/Battery_Temp?Temperature=${battery_hdc_temp}"
				fi
			fi
		fi
		...
		if [ "x$pth_pressure" != "x" ] ; then
			if [ 0 -eq "$(echo "${pth_pressure} > ${max_pressure}" | bc)" ] ; then
				...
				if [ -f /home/voodoo/send.data ] ; then
					curl -X POST "http://192.168.11.252:8100/v1/data/for/pth_pressure?pressure=${pth_pressure}"
				fi
			fi
		fi
		...
		if [ "x$pth_temp" != "x" ] ; then
			if [ 0 -eq "$(echo "${pth_temp} > ${max_temperature}" | bc)" ] ; then
				...
				if [ -f /home/voodoo/send.data ] ; then
					curl -X POST "http://192.168.11.252:8100/v1/data/for/pth_temp?temp=${pth_temp}"
				fi
			fi
		fi
		...
		if [ "x$pth_humidity" != "x" ] ; then
			...
			if [ -f /home/voodoo/send.data ] ; then
				curl -X POST "http://192.168.11.252:8100/v1/data/for/pth_humidity?humidity=${pth_humidity}"
			fi
		fi
