...
#MsgFrom:0013A20041A7AE31:12V_RAIL:341754561:Current:-1.0mA*
	...
	do
		...
		solar_volt=$(echo $READ | grep -a '0013A20041A7AE31:Solar:' | grep -a BusVolt | tail -1 | awk -F ':' '{print $6}' | awk -F 'V*' '{print $1}' || true)
		echo "solar_volt=[$solar_volt]"
		solar_load=$(echo $READ | grep -a '0013A20041A7AE31:Solar:' | grep -a Current | tail -1 | awk -F ':' '{print $6}' | awk -F 'mA*' '{print $1}' || true)
